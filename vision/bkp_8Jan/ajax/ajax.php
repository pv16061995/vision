...
function successmarket() 
{
  ...
  $i=0;
    ...
    foreach($repon as $data)
    {
      if(isset($data['bidowner'.$currency1])  )
      {

      if($data['status']=='1' && $data['bidowner'.$currency1]==$id )
      {
      ...
      if( isset($data['askowner'.$currency1])  )
      {

      if($data['status']=='1' && $data['askowner'.$currency1]==$id)
      {

        if(isset($data['askAmount'.$currency1])){
          $detail .='<tr>';
          $detail .='<td>'.date('d-M-Y h:i:s',strtotime($data['createdAt'])).'</td>';
          $detail .='<td>ASK</td>';
          $detail .='<td>'.$data['askAmount'.$currency1].'</td>';
